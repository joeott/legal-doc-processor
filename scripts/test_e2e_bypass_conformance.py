#!/usr/bin/env python3
"""
Test end-to-end processing of a single document - bypassing conformance validation
"""
...
import scripts.db
original_validate = scripts.db.DatabaseManager.validate_conformance
...
from scripts.rds_utils import select_records, update_record
...
def find_test_document():
    ...
    query = """
        SELECT document_uuid, original_file_name, s3_key, status, celery_status
        FROM source_documents
        WHERE detected_file_type = 'application/pdf'
    ...
    if results:
        ...
        for doc in results:
            logger.info(f"  - {doc['document_uuid']}: {doc['original_file_name']} (status: {doc['status']}, celery: {doc['celery_status']})")
...
def monitor_task_progress(task_result):
    ...
    while True:
        ...
        if doc_status:
            doc = doc_status[0]
            logger.info(f"Document status: {doc['status']} | Celery status: {doc['celery_status']}")
...
def check_worker_status():
    ...
    redis_manager = get_redis_manager()
    logger.info(f"Redis connection: {'✓ Connected' if redis_manager.get_client().ping() else '✗ Failed'}")
    
    # Check registered tasks
    logger.info(f"Registered tasks: {len([t for t in app.tasks if not t.startswith('celery.')])}")
...
def main():
    """Main test function"""
    logger.info("=== Starting End-to-End Document Processing Test (Conformance Bypass) ===")
    logger.warning("⚠️  Conformance validation temporarily bypassed!")
    ...
    if not document_uuid:
        ...
        from scripts.rds_utils import execute_query
        any_docs = execute_query("SELECT document_uuid FROM source_documents LIMIT 1")
        if any_docs:
    ...
    if task_result:
        ...
        if final_status:
            ...
            logger.info(f"Processing Status: {doc['status']}")
