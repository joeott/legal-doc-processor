...
os.environ['S3_BUCKET_REGION'] = 'us-east-2'  # Critical: Set correct region
...
class PipelineTestRunner:
    ...
    def run_full_test(self):
        ...
        print("="*80)
        print("FULL PIPELINE TEST - POST S3 PERMISSIONS & REGION FIX")
        print("="*80)
        print(f"Start time: {datetime.now()}")
        print(f"S3 Bucket Region: us-east-2")
        print(f"S3 Bucket: {self.s3_manager.private_bucket_name}")
    ...
    def test_ocr_submission(self, doc_uuid):
        ...
        try:
            ...
            print(f"   Using region: us-east-2")
        ...
        while time.time() - start_time < timeout:
            ...
            for stage, status in current_state.items():
                if stage not in last_state or last_state[stage] != status:
                    state_changed = True
                    if status != 'none' and status != last_state.get(stage):
                        print(f"\n[{int(time.time() - start_time):3d}s] 🔄 {stage}: {last_state.get(stage, 'none')} → {status}")
            ...
            if state_changed:
                ...
                print(f"{status_line} | OCR→Chunk→Entity→Resolve→Relations")
            ...
            if failed_stages:
                ...
                for stage in failed_stages:
                    state_key = f"doc:state:{doc_uuid}:{stage}"
                    stage_data = self.redis_manager.get_dict(state_key) or {}
                    if 'metadata' in stage_data and 'error' in stage_data['metadata']:
                        print(f"   {stage} error: {stage_data['metadata']['error']}")
    ...
    def verify_data_quality(self, doc_uuid):
        ...
        try:
            ...
            for session in self.db_manager.get_session():
                ...
                metrics['chunks'] = {'count': chunk_count or 0, 'avg_words': float(avg_words or 0)}
                ...
                metrics['entities'] = {'mentions': mention_count or 0, 'canonical': canonical_count or 0}
    ...
    def generate_report(self):
        ...
        if passed_tests == total_tests:
            ...
            print("🚀 The legal document processing system is PRODUCTION READY!")
        else:
