...
def test_project_model_minimal_data():
    ...
    assert project.processedByScripts is False
...
def test_project_model_to_db_dict():
    ...
    assert db_dict["scriptRunCount"] == data_with_none["scriptRunCount"]
    ...
    assert db_dict_no_alias["script_run_count"] == data_with_none["scriptRunCount"]
...
def test_source_document_model_minimal_data():
    ...
    assert doc.ocr_metadata_json == {} # Default from validator
    assert doc.transcription_metadata_json == {} # Default from validator
...
def test_source_document_model_validation_errors():
    ...
    assert "UUID input should be a UUID instance or a string representation of a UUID" in str(excinfo_proj_uuid.value)
...
def test_chunk_model_required_fields():
    ...
    assert chunk.metadata_json == {} # Default from validator/field
...
def test_chunk_model_to_db_dict():
    ...
    assert db_dict_no_alias["next_chunk_id"] is None # Field name
...
def test_entity_mention_model_required_fields():
    ...
    assert entity_mention.created_at is not None
    assert entity_mention.updated_at is not None
    assert entity_mention.attributes_json == {} # Default from field/validator
...
def test_entity_mention_model_to_db_dict():
    ...
    assert db_dict_no_alias["offset_start"] is None
...
def test_project_model_all_fields():
    ...
    assert project.scriptRunCount == data["scriptRunCount"]
    assert project.processedByScripts == data["processedByScripts"]
...
def test_project_model_default_values():
    """Test default values for scriptRunCount, processedByScripts, and active."""
    project = ProjectModel(name="Test Project")
    assert project.scriptRunCount == 0
    assert project.processedByScripts is False
    assert project.active is True
